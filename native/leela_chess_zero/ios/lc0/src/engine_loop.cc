...
void RunEngine(SearchFactory* factory) {
  ...
  // Run the private FFI pipe loop. std::cin is process-wide and is also used
  // by the in-process Stockfish plugin.
  std::string line;
  while (ReadMobileUciInput(&line)) {
    LOGFILE << ">> " << line;
    try {
